...
sed -i "s/<Context>/<Context sessionCookieName=\"JSESSIONID_$HOSTNAME\">/" /etc/tomcat8/context.xml

# Make sure Tomcat has enough memory
sed -i "s/JAVA_OPTS=\"-Djava.awt.headless=true -Xmx128m -XX:+UseConcMarkSweepGC\"/JAVA_OPTS=\"-Djava.awt.headless=true -Xms256m -Xmx1024m -XX:MaxPermSize=128m -server -XX:+UseConcMarkSweepGC\"/" /etc/default/tomcat8 
...
ln -s /var/lib/tomcat8/webapps/ROOT/ /root/opencms
