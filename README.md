...
These official Dockerfiles install various types of **OpenCms 9.5.1** systems with demo content. 
...
*Latest supported OpenCms version: 9.5.1*
...
```Shell
cd ~/opencms-docker/9.5.1-simple
docker build -t alkacon/opencms-docker:9.5.1-simple .
docker run -d -p 8080:8080 -p 22000:22 alkacon/opencms-docker:9.5.1-simple
```
...
```Shell
cd ~/opencms-docker/9.5.1-dev
docker build -t alkacon/opencms-docker:9.5.1-dev .
docker run -d -p 80:8080 -p 22000:22 -p 445:1445 alkacon/opencms-docker:9.5.1-dev
```
